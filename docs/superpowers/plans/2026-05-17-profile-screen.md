# Profile Screen — Real User Data Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the bare-bones Profile tab with a screen showing a full identity card (name, school, grade, listing), a Google account row when signed in, and the existing action cards; also delete the orphaned SplashOverlay component.

**Architecture:** `profile.tsx` reads from local SQLite only — two sequential `db.select()` calls inside a `useEffect` (load `userSettings`, then optionally load `listings` for the listing title). No new hook needed. Follows the same inline-query pattern used in `apps/mobile/app/settings.tsx`. This is a pure rendering component with no testable pure functions, so verification is TypeScript type-check instead of unit tests.

**Tech Stack:** Expo 54 · Expo Router v4 · expo-sqlite + Drizzle ORM · `@lineiconshq/react-native-lineicons` (`User4Outlined`, `SparkOutlined`) · Outfit + Lexend fonts · StyleSheet (no NativeWind)
...
| File | Action |
|---|---|
| `apps/mobile/app/(tabs)/profile.tsx` | Replace — full redesign |
| `apps/mobile/components/SplashOverlay.tsx` | Delete — orphaned dead code |

---

## Task 1: Delete SplashOverlay.tsx

**Files:**
- Delete: `apps/mobile/components/SplashOverlay.tsx`

- [ ] **Step 1: Verify SplashOverlay is not imported anywhere**

```bash
cd apps/mobile && grep -r "SplashOverlay" --include="*.ts" --include="*.tsx" .
```
...
```bash
rm apps/mobile/components/SplashOverlay.tsx
```
...
```bash
git rm apps/mobile/components/SplashOverlay.tsx
git commit -m "chore(mobile): delete orphaned SplashOverlay component"
```

---

## Task 2: Profile screen redesign

**Files:**
- Modify: `apps/mobile/app/(tabs)/profile.tsx`

- [ ] **Step 1: Replace the full contents of `apps/mobile/app/(tabs)/profile.tsx`**
