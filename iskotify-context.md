...
* **Supabase migrations are applied manually** by pasting the SQL files in `supabase/migrations/` into the Supabase SQL editor, in numeric order, or through the Supabase MCP (pinned to this project) since 2026-09-24. Latest applied: `056` (function search_path pins + early_access_status grant revoke). A `CREATE OR REPLACE FUNCTION` drops any `SET search_path` the function had unless it repeats the clause. Migrations run in the SQL editor are not recorded in `supabase_migrations`, so check the schema, not the migration list.
