...
* **Supabase migrations are applied manually** by pasting the SQL files in `supabase/migrations/` into the Supabase SQL editor, in numeric order, or through the Supabase MCP (pinned to this project) since 2026-09-24. Latest applied: `062` (bug-report screenshots bucket is private; admins view them through 10-minute signed URLs from `/api/admin/app-reports/[id]/screenshot`). `061` added the updated_at indexes backing the mobile delta sync and a GIN index on `flashcards.listing_slugs`. `058` adds `kb_drive_files.missing_figures`, `059` `question_flag_dismissals`, `060` RLS initplan fixes and FK indexes; `057` dropped the estimate_admission_score RPC (scoring is on-device). A `CREATE OR REPLACE FUNCTION` drops any `SET search_path` the function had unless it repeats the clause. Migrations run in the SQL editor are not recorded in `supabase_migrations`, so check the schema, not the migration list.
...
* **Estimated Admission Score** — `app/estimator/` (entry: Home and Practice cards). Computed on the device by `utils/admissionEstimate.ts` (published Manlapaz regression, static baselines, EEAS palugit/pabigat, ±0.20 range, lower is better) from the student's Grade 8–11 GWA and their last 60 answers per UPCAT subtest in `question_attempts` (`utils/subtestReadiness.ts`; a subtest counts after 20 answers — no stand-in averages). Campus status uses `upcat_cutoffs` (synced; 2019 campus-level estimates plus a few program rows). UP OAA rules are enforced by `app/estimator/__tests__/compliance.test.tsx`: "Estimated Admission Score", "based on historical cutoffs", never "your UPG is" / "will qualify" / pass-fail, no UP branding, and the EN/TL disclaimer must be accepted before first view.
