...
export function StatCard({ label, value, sub, accent, onClick, active }: Props) {
  ...
  const clickableClass = onClick
    ...
    : ''
  const activeClass = active ? 'ring-2 ring-maroon/30 bg-[#fff8f8]' : ''
  ...
  if (onClick) {
    return (
      ...
      >
        <p className="text-[10px] font-semibold text-ink-subtle uppercase tracking-widest mb-2">{label}</p>
        <p className={`font-heading font-extrabold text-[2rem] leading-none tracking-tight mb-1 ${accent ?? 'text-ink'}`}>
          {value}
        </p>
