...
export function ApkUrlForm({ currentUrl }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/early-access/apk-url', {
        method: 'POST',
      ...
      if (!json.ok) {
        ...
        setStatus({ type: 'success', message: trimmed === '' ? 'APK link cleared.' : 'APK link saved successfully.' })
        router.refresh()
      }
    } catch {
  ...
  return (
    <div className="rounded-[12px] border border-black/[0.07] bg-[#fafafa] px-4 py-4 space-y-3">
      ...
      <div className="space-y-2">
        <label
          htmlFor="apk-url-input"
          className="block text-[13px] text-[#6e6e73]"
        >
          Paste the permanent hosted download URL (GitHub Releases, Google Drive, etc.)
        </label>
        <input
          id="apk-url-input"
          type="url"
