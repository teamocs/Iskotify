  ...
  if (listings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="font-heading font-bold text-lg text-ink">No listings yet</p>
        <p className="text-sm text-ink-muted mt-1">Check back soon — new opportunities are added weekly.</p>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {displayList.length === 0 ? (
          <p className="col-span-full text-center text-ink-muted py-12">No listings match your filter.</p>
        ) : (
      ...
      {hasMore && (
        <div className="flex justify-center pb-8">
          <Link
            href="/listings"
            className="inline-flex items-center gap-2 bg-maroon text-white rounded-[980px] px-6 py-3 text-sm font-medium font-body hover:bg-maroon-light transition-colors shadow-sm"
          >
            View All Listings →
          </Link>
        </div>
