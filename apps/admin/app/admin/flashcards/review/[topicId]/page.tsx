    ...
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6 max-w-5xl mx-auto space-y-6">
          ...
          <div>
            <div className="text-ink-muted text-xs font-semibold uppercase tracking-wider mb-1">
              {topic?.subject_name ?? '—'}
            </div>
          ...
          <section className="space-y-3">
            <h3 className="text-ink text-[13px] font-semibold uppercase tracking-wider font-heading">
              Cards
            </h3>
            {loading ? (
              <div className="rounded-2xl border border-black/[0.08] bg-white px-6 py-12 text-center text-ink-muted text-sm shadow-sm">
                Loading cards…
              </div>
            ) : cards.length === 0 ? (
              <div className="rounded-2xl border border-black/[0.08] bg-white px-6 py-12 text-center text-ink-muted text-sm shadow-sm">
                  ...
                  return (
                    <div key={c.id} className="rounded-2xl border border-black/[0.08] bg-white p-5 shadow-sm">
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-ink-muted text-xs font-medium">Card {i + 1}</div>
                        {hasAi && (
                      ...
                      <div className="text-ink font-medium mb-2 leading-snug">{c.question}</div>
                      <div className="text-ink text-sm mb-3">
                        <span className="text-ink-muted">Answer: </span>
                        <span className="text-success font-medium">{c.answer}</span>
                      </div>
                      {opts.length > 0 ? (
                        <ul className="text-sm space-y-1 mt-2 pl-1">
                          {opts.map((o, j) => (
                            <li
                              key={j}
                              className={j === correct ? 'text-success font-medium' : 'text-ink-muted'}
                            >
                        ...
                        <div className="inline-flex items-center px-2.5 py-1 rounded-full bg-warning-soft text-warning-strong text-xs font-medium">
                          ⏳ No distractors yet — Gemini will fill in shortly
                        </div>
                      )}
                      {(hasAi ? c.ai_explanation : c.explanation) && (
                        <div className="text-ink-muted text-xs mt-3 leading-relaxed border-t border-black/[0.06] pt-2">
